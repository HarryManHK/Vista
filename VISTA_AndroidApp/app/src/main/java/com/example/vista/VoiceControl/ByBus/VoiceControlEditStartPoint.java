...
import com.example.vista.FindBusEditMenuFunction.EditDestinationActivity;
...
public class VoiceControlEditStartPoint extends AppCompatActivity {

    private com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.DownloadTask task = null;
    private ListView lvShowAllStop;
    private ArrayList<com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop> busStops; // Custom class to hold bus stop details
    ...
    private static final int REQ_CODE_SPEECH_INPUT = 3001;
    private siliconflow_Deepseek_NLP nlpService;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_voice_control_edit_start_point);
        ...
        String[] languageSetting = SettingDBHelper.getLanguageSetting();
        ...
        lvShowAllStop.setOnItemClickListener((parent, view, position, id) -> {
            ...
            com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop selectedBusStop = busStops.get(position);
        ...
        btnConfirm.setOnClickListener(v -> {
            if (selectedPosition != -1) {
                ...
                com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop selectedBusStop = busStops.get(selectedPosition);
                ...
                Intent intent = new Intent(com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.this, VoiceControlEditEndPoint.class);
                startActivity(intent);
                finish();
            } else {
                Toast.makeText(com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.this, "Please select a start point bus stop", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        if (BusRoute != null && !BusRoute.isEmpty() && bound != null && !bound.isEmpty()) {
            if (task == null || task.getStatus() == AsyncTask.Status.FINISHED) {
                task = new com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.DownloadTask();
    ...
    private void selectNextItem() {
        ...
        com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop selectedBusStop = busStops.get(nextPosition);
        tts.speak(new String[]{selectedBusStop.getNameEn(), selectedBusStop.getNameZH()});
    }
     ...
     */
    private class DownloadTask extends AsyncTask<String, Void, ArrayList<com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop>> {
        @Override
        protected ArrayList<com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop> doInBackground(String... values) {
            String apiUrl = values[0];
            ArrayList<com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop> stopsList = new ArrayList<>();
            try {
                ...
                for (int i = 0; i < dataArray.length(); i++) {
                    ...
                    com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop busStop = new com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop(stopId, nameEn, nameZH, seq, lat, lon);
        ...
        @Override
        protected void onPostExecute(ArrayList<com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop> result) {
            if (result == null) {
                Toast.makeText(com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.this, "Failed to fetch bus stops data", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            for (com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop stop : busStops) {
            ...
            adapter = new BusStopListViewAdapter(com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.this,
    ...
    private void promptSpeechInput() {
        ...
        intent.putExtra(android.speech.RecognizerIntent.EXTRA_PROMPT, "請說出欲上車的巴士站");
        try {
    ...
    private void sendBusStopsToNLP(String userSpeech, ArrayList<BusStop> busStops) {
        ...
        nlpService.processText(nlpPrompt, new NLPCallback() {
            ...
            public void onFailure(Exception e) {
                runOnUiThread(() -> Toast.makeText(VoiceControlEditStartPoint.this, "NLP服務異常", Toast.LENGTH_SHORT).show());
            }
        });
    }
     ...
     */
    private void updateDatabase(com.example.vista.VoiceControl.ByBus.VoiceControlEditStartPoint.BusStop selectedBusStop) {
